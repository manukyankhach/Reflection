...
namespace ReflactionsExamples
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // GetMethods()
            Type getMethods = typeof(Person);
            MethodInfo[] methodInfo = getMethods.GetMethods();
            Console.WriteLine($"IsFamily - {methodInfo[0].IsFamily}, IsFamilyOrAssembly - {methodInfo[1].IsFamilyOrAssembly}," +
                $" IsPrivate - {methodInfo[2].IsPrivate}, IsPublic - {methodInfo[3].IsPublic}, Name - {methodInfo[4].Name}");
            Console.ReadLine();
        }
    }
}
